...
describe("componentDidMount", () => {
  it("should call tryToAuthenticateWithOIDC", () => {
    ...
    shallow(<BYOCView {...defaultProps} tryToAuthenticateWithOIDC={tryToAuthenticateWithOIDC} />);
...
it("renders a token login form", () => {
  const wrapper = shallow(<BYOCView {...defaultProps} />);
...
describe("redirect if authenticated", () => {
  it("redirects to / if no current location", () => {
    const wrapper = shallow(<BYOCView {...defaultProps} authenticated={true} />);
  ...
  it("redirects to previous location", () => {
    ...
    const wrapper = shallow(
      <BYOCView {...defaultProps} authenticated={true} location={location} />,
    );
    const redirect = wrapper.find(Redirect);
    expect(redirect.exists()).toBe(true);
...
it("displays an error if the authentication error is passed", () => {
  const wrapper = shallow(<BYOCView {...defaultProps} authenticationError={authenticationError} />);
