...
describe("componentDidMount", () => {
  it("should call tryToAuthenticateWithOIDC", () => {
    ...
    shallow(<Profile {...defaultProps} tryToAuthenticateWithOIDC={tryToAuthenticateWithOIDC} />);
...
describe("redirect if authenticated", () => {
  it("redirects to / if no current location", () => {
    const wrapper = shallow(<Profile {...defaultProps} authenticated={true} />);
  ...
  it("redirects to previous location", () => {
    ...
    const wrapper = shallow(<Profile {...defaultProps} authenticated={true} location={location} />);
    const redirect = wrapper.find(Redirect);
    expect(redirect.exists()).toBe(true);
...
it("displays an error if the authentication error is passed", () => {
  const wrapper = shallow(<Profile {...defaultProps} authenticationError={authenticationError} />);
